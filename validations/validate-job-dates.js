...
export const validateJobAndDates = (record) => {
  try {
    ...
    if (isNil(title) && isNotNil(code)) {
      ...
      const jobTitle =
        linkedJob && linkedJob.length > 0 ? linkedJob[0]?.title : null
    ...
    if (isNil(code) && isNotNil(title)) {
      ...
      const jobCode =
        linkedJob && linkedJob.length > 0 ? linkedJob[0]?.code : null
      record.set('Job_Profile_Reference_ID', jobCode)
      record.addInfo(
        'Job_Profile_Reference_ID',
        'Code pulled from job profile.'
      )
    }
    ...
    if (isNotNil(code) && isNotNil(title)) {
      ...
      const jobCode =
        linkedJobfromTitle && linkedJobfromTitle.length > 0
          ? linkedJobfromTitle[0]?.code
          : null
      const linkedJobfromCode = record.getLinks('Job_Profile_Reference_ID')
      const jobTitle =
        linkedJobfromCode && linkedJobfromCode.length > 0
          ? linkedJobfromCode[0]?.title
          : null
      if (jobCode !== code || jobTitle !== title) {
        record.addWarning(
          ['Job_Profile_Reference_ID', 'Job_Title'],
          'Job code and title match different jobs from the list of codes you previously entered.'
        )
      }
    }
    ...
    if (empType === 'CW' || terminated === true) {
      if (isNil(endEmploymentDate)) {
        const message =
          'Temp or Terminated Employees must have an Employment End Date, auto-set to 2099-12-31'
      ...
      if (isNotNil(endEmploymentDate)) {
        const message =
          'Employment End Date is only valid for Temp or Terminated Employees'
        record.addError('End_Employment_Date', message)
      }
    }
    ...
    const payRateFromCode =
      codeLookup && codeLookup.length > 0 ? codeLookup[0]?.pay_rate_type : null
    if (!payRate) {
      record.set('Pay_Rate_Type_Reference_ID', payRateFromCode)
    } else {
      if (payRate !== payRateFromCode) {
        const message =
          "Pay Rate doesn't match the pay rate from the linked job code."
