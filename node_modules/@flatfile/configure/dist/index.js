...
__export(src_exports, {
  ...
  Portal: () => Portal,
  RecordTranslater: () => RecordTranslater,
...
var Action = class {
  constructor(nameOrOptions, handler) {
    ...
    if (typeof nameOrOptions === "string") {
      ...
      this.label = nameOrOptions.label || nameOrOptions.slug || nameOrOptions.operation || "Unknown Action";
...
var AuthenticatedClient = class {
  ...
  async fetch(url, options = {}) {
    ...
    const headers = {
      Authorization: `Bearer ${process.env.FLATFILE_BEARER_TOKEN}` ?? `Bearer ...`,
      "x-disable-hooks": "true",
      ...options.headers
    };
...
var FlatfileEvent = class extends AuthenticatedClient {
  ...
  constructor(src) {
    ...
    this.name = src.topic;
    this.target = this.domain === "file" ? "space(*)" : actionName ? actionTarget : `sheet(${sheetSlug?.split("/").pop()})`;
...
var Workbook = class extends EventHandler {
  constructor(options) {
    ...
    this.processRecords = async (records, payload, logger) => {
...
var Sheet = class extends EventHandler {
   ...
   * @param e
   */
  async recordsChangedEventShim(e) {
...
var SheetTester = class {
  ...
  async transformRecord(recordBatch) {
    const session = new import_hooks3.FlatfileSession({
      ...this.testSession,
  ...
  async transformRecords(recordBatch) {
    const session = new import_hooks3.FlatfileSession({
      ...this.testSession,
      schemaSlug: this.sheetName
    });
  ...
  async transformField(pkey, value, other = {}, errOn = "error") {
    ...
    const transformRecord = async (override) => this.transformRecord({
      ...defaultCastMap,
      ...override
    });
    const transform2 = async (pkey2, value2) => {
      const key = labelKeyMap[pkey2] ?? pkey2;
      const out = await transformRecord({ [key]: value2, ...other });
...
function makeField(baseField, newDefaults = {}, customizer = (mergedOptions, ...args) => new Field(mergedOptions)) {
  const fieldMaker = (newOptions) => {
    ...
    const typedBaseOpts = {
      ...GenericDefaults,
      // only needed because missing field key
      ...hookDefaults
    };
